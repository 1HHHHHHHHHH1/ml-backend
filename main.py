...
def predict(req: PredictRequest):
    ...
    explanation = f"{dec} - {prob * 100:.1f}% with {req.investor_name}. "
    explanation += (
        f"Signals: {', '.join(signals[:3])}." if signals else "No strong signals."
    )
    if description_score > 0:
        explanation += f" Description alignment: {description_score * 100:.0f}%."
        ...
        try:
            ...
            out.append(
                BulkResult(
                    ...
                    match_percentage=round(prob * 100, 1),
                    confidence_level=_conf(prob),
                    positive_signals=_positive_signals(pos, single, description_score),
                    negative_signals=neg,
                )
            )
        except Exception as e:
...
def _positive_signals(pos: List[str], req: PredictRequest, description_score: float):
    signals = list(pos)
    if description_score >= 0.35 and "description alignment" not in signals:
...
def _text_cosine_similarity(left: str, right: str):
    ...
    try:
        vectors = tfidf.transform([left, right]).toarray()
        return _cosine(vectors[0], vectors[1])
    except Exception:
        return 0.0


def _token_overlap(left: str, right: str):
...
def _conf(probability: float):
    ...
    if probability >= 0.60 or probability <= 0.40:
        return "Medium"
    return "Low"
>>>>>>> 0933bdd (Add project and investor description alignment to matching)
